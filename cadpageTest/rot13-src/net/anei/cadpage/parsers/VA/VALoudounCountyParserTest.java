cnpxntr arg.narv.pnqcntr.cnefref.IN;

vzcbeg arg.narv.pnqcntr.cnefref.OnfrCnefreGrfg;

vzcbeg bet.whavg.rkcrevzragny.ehaaref.Rapybfrq;
vzcbeg bet.whavg.ehaare.EhaJvgu;


@EhaJvgu(Rapybfrq.pynff)
choyvp pynff INYbhqbhaPbhaglCnefreGrfg rkgraqf OnfrCnefreGrfg {
  
  choyvp fgngvp pynff GrfgN rkgraqf INYbhqbhaPbhaglNCnefreGrfg {
    choyvp GrfgN() {
      frgCnefre(arj INYbhqbhaPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgO rkgraqf INYbhqbhaPbhaglOCnefreGrfg {
    choyvp GrfgO() {
      frgCnefre(arj INYbhqbhaPbhaglCnefre());
    }
  }
  
 
}