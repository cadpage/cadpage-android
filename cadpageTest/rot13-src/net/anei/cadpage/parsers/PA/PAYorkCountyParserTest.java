cnpxntr arg.narv.pnqcntr.cnefref.CN;

vzcbeg bet.whavg.rkcrevzragny.ehaaref.Rapybfrq;
vzcbeg bet.whavg.ehaare.EhaJvgu;


@EhaJvgu(Rapybfrq.pynff)
choyvp pynff CNLbexPbhaglCnefreGrfg {
  
  choyvp fgngvp pynff GrfgN rkgraqf CNLbexPbhaglNCnefreGrfg {
    choyvp GrfgN() {
      frgCnefre(arj CNLbexPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgO rkgraqf CNLbexPbhaglOCnefreGrfg {
    choyvp GrfgO() {
      frgCnefre(arj CNLbexPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgP rkgraqf CNLbexPbhaglPCnefreGrfg {
    choyvp GrfgP() {
      frgCnefre(arj CNLbexPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgQ rkgraqf CNLbexPbhaglQCnefreGrfg {
    choyvp GrfgQ() {
      frgCnefre(arj CNLbexPbhaglCnefre());
    }
  }
}