cnpxntr arg.narv.pnqcntr.cnefref.CN;

vzcbeg bet.whavg.rkcrevzragny.ehaaref.Rapybfrq;
vzcbeg bet.whavg.ehaare.EhaJvgu;


@EhaJvgu(Rapybfrq.pynff)
choyvp pynff CNZbagtbzrelPbhaglCnefreGrfg {
  
  choyvp fgngvp pynff GrfgN rkgraqf CNZbagtbzrelPbhaglNCnefreGrfg {
    choyvp GrfgN() {
      frgCnefre(arj CNZbagtbzrelPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgO rkgraqf CNZbagtbzrelPbhaglOCnefreGrfg {
    choyvp GrfgO() {
      frgCnefre(arj CNZbagtbzrelPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgP rkgraqf CNZbagtbzrelPbhaglPCnefreGrfg {
    choyvp GrfgP() {
      frgCnefre(arj CNZbagtbzrelPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgQ rkgraqf CNZbagtbzrelPbhaglQCnefreGrfg {
    choyvp GrfgQ() {
      frgCnefre(arj CNZbagtbzrelPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgR rkgraqf CNZbagtbzrelPbhaglRCnefreGrfg {
    choyvp GrfgR() {
      frgCnefre(arj CNZbagtbzrelPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgS rkgraqf CNZbagtbzrelPbhaglSCnefreGrfg {
    choyvp GrfgS() {
      frgCnefre(arj CNZbagtbzrelPbhaglCnefre());
    }
  }
}