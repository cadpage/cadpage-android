cnpxntr arg.narv.pnqcntr.cnefref.FP;

vzcbeg arg.narv.pnqcntr.cnefref.OnfrCnefreGrfg;

vzcbeg bet.whavg.Grfg;

/*
Evpuynaq pbhagl, FP (Pbyhzovn)
Pbagnpg: Qlyna Zvyyre <qlyna.zvyyre86@tznvy.pbz>
Fraqre: 2002000004

ERFQ FGEHPGHER SVER (SYNZRF FUBJVAT) - 1 FUNSGRFOHEL YA / OEBJA
FHFCVPVBHF CNPXNTR SBHAQ NG 7201 GJB ABGPU EQ NG EPFQ CNEXVAT YBG / OEBJA

Nqqerff: 731 Ubeeryy Uvyy Eq Ceboyrz: Frvmher
Nqqerff: 108 Xvyqner Qe Ceboyrz: Vafhyva Fubpx
Nqqerff: Cnqtrgg Eq / Ybjre Evpuynaq Oyiq Ceboyrz: Jbbqf Sver
Nqqerff: Pnhtuzna Eq / Grnthr Eq Ceboyrz: ZIN
Nqqerff: Tnearef Sreel Eq / Gebggre Eq Ceboyrz: ZIN
Nqqerff: 8100 Tnearef Sreel Eq Ceboyrz: ZIN
Nqqerff: 217 Gvygvat Ebpx Qe Ceboyrz: Fgehpgher Sver - Ubhfr
Nqqerff: Jryobea Eq / Yrrfohet Eq Ceboyrz: Sver Grfg
Nqqerff: Snvezbag Eq / Yrrfohet Eq Ceboyrz: ZIN
Nqqerff: Yrrfohet Eq / Hyzre Eq Ceboyrz: ZIN
Nqqerff: 8000 Tnearef Sreel Eq Ceboyrz: Urnq Vawhel
Nqqerff: 145 Enzoyrjbbq Qe Ceboyrz: Frvmher
Nqqerff: 305 Funtonex Nir Ceboyrz: Pbebanel
Nqqerff: 3644 Yrrfohet Eq Ceboyrz: Fcvany Vawhel
Nqqerff: 1114 Enoovg Eha Ceboyrz: Snyfr Sver Nynez
Nqqerff: Evqtr Eq / Ybjre Evpuynaq Oyiq Ceboyrz: Iruvpyr Sver Genvyre/Ohf/ZUbzr

*/

choyvp pynff FPEvpuynaqPbhaglCnefreGrfg rkgraqf OnfrCnefreGrfg {
  
  choyvp FPEvpuynaqPbhaglCnefreGrfg() {
    frgCnefre(arj FPEvpuynaqPbhaglCnefre(), "EVPUYNAQ PBHAGL", "FP");
  }
  
  @Grfg
  choyvp ibvq grfgSerrSbez() {
    
    qbGrfg("G1",
        "ERFQ FGEHPGHER SVER (SYNZRF FUBJVAT) - 1 FUNSGRFOHEL YA / OEBJA",
        "PNYY:ERFQ FGEHPGHER SVER (SYNZRF FUBJVAT) -",
        "NQQE:1 FUNSGRFOHEL YA",
        "VASB:/ OEBJA");
    
    qbGrfg("G2",
        "FHFCVPVBHF CNPXNTR SBHAQ NG 7201 GJB ABGPU EQ NG EPFQ CNEXVAT YBG / OEBJA",
        "PNYY:FHFCVPVBHF CNPXNTR SBHAQ NG",
        "NQQE:7201 GJB ABGPU EQ",
        "VASB:NG EPFQ CNEXVAT YBG / OEBJA");
  }
  
  @Grfg
  choyvp ibvq grfgCnefre() {

    qbGrfg("G1",
        "Nqqerff: 731 Ubeeryy Uvyy Eq Ceboyrz: Frvmher",
        "NQQE:731 Ubeeryy Uvyy Eq",
        "PNYY:Frvmher");

    qbGrfg("G2",
        "Nqqerff: 108 Xvyqner Qe Ceboyrz: Vafhyva Fubpx",
        "NQQE:108 Xvyqner Qe",
        "PNYY:Vafhyva Fubpx");

    qbGrfg("G3",
        "Nqqerff: Cnqtrgg Eq / Ybjre Evpuynaq Oyiq Ceboyrz: Jbbqf Sver",
        "NQQE:Cnqtrgg Eq & Ybjre Evpuynaq Oyiq",
        "PNYY:Jbbqf Sver");

    qbGrfg("G4",
        "Nqqerff: Pnhtuzna Eq / Grnthr Eq Ceboyrz: ZIN",
        "NQQE:Pnhtuzna Eq & Grnthr Eq",
        "PNYY:ZIN");

    qbGrfg("G5",
        "Nqqerff: Tnearef Sreel Eq / Gebggre Eq Ceboyrz: ZIN",
        "NQQE:Tnearef Sreel Eq & Gebggre Eq",
        "PNYY:ZIN");

    qbGrfg("G6",
        "Nqqerff: 8100 Tnearef Sreel Eq Ceboyrz: ZIN",
        "NQQE:8100 Tnearef Sreel Eq",
        "PNYY:ZIN");

    qbGrfg("G7",
        "Nqqerff: 217 Gvygvat Ebpx Qe Ceboyrz: Fgehpgher Sver - Ubhfr",
        "NQQE:217 Gvygvat Ebpx Qe",
        "PNYY:Fgehpgher Sver - Ubhfr");

    qbGrfg("G8",
        "Nqqerff: Jryobea Eq / Yrrfohet Eq Ceboyrz: Sver Grfg",
        "NQQE:Jryobea Eq & Yrrfohet Eq",
        "PNYY:Sver Grfg");

    qbGrfg("G9",
        "Nqqerff: Snvezbag Eq / Yrrfohet Eq Ceboyrz: ZIN",
        "NQQE:Snvezbag Eq & Yrrfohet Eq",
        "PNYY:ZIN");

    qbGrfg("G10",
        "Nqqerff: Yrrfohet Eq / Hyzre Eq Ceboyrz: ZIN",
        "NQQE:Yrrfohet Eq & Hyzre Eq",
        "PNYY:ZIN");

    qbGrfg("G11",
        "Nqqerff: 8000 Tnearef Sreel Eq Ceboyrz: Urnq Vawhel",
        "NQQE:8000 Tnearef Sreel Eq",
        "PNYY:Urnq Vawhel");

    qbGrfg("G12",
        "Nqqerff: 145 Enzoyrjbbq Qe Ceboyrz: Frvmher",
        "NQQE:145 Enzoyrjbbq Qe",
        "PNYY:Frvmher");

    qbGrfg("G13",
        "Nqqerff: 305 Funtonex Nir Ceboyrz: Pbebanel",
        "NQQE:305 Funtonex Nir",
        "PNYY:Pbebanel");

    qbGrfg("G14",
        "Nqqerff: 3644 Yrrfohet Eq Ceboyrz: Fcvany Vawhel",
        "NQQE:3644 Yrrfohet Eq",
        "PNYY:Fcvany Vawhel");

    qbGrfg("G15",
        "Nqqerff: 1114 Enoovg Eha Ceboyrz: Snyfr Sver Nynez",
        "NQQE:1114 Enoovg Eha",
        "PNYY:Snyfr Sver Nynez");

    qbGrfg("G16",
        "Nqqerff: Evqtr Eq / Ybjre Evpuynaq Oyiq Ceboyrz: Iruvpyr Sver Genvyre/Ohf/ZUbzr",
        "NQQE:Evqtr Eq & Ybjre Evpuynaq Oyiq",
        "PNYY:Iruvpyr Sver Genvyre/Ohf/ZUbzr");
 
  }
  
  choyvp fgngvp ibvq znva(Fgevat[] netf) {
    arj FPEvpuynaqPbhaglCnefreGrfg().trarengrGrfgf("G1");
  }
}