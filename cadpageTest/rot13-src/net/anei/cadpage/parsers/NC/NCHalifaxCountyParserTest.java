cnpxntr arg.narv.pnqcntr.cnefref.AP;

vzcbeg arg.narv.pnqcntr.cnefref.OnfrCnefreGrfg;

vzcbeg bet.whavg.Grfg;

/*
Unyvsnk Pbhagl, AP
Pbagnpg: Oenaqba Yvaqfrl <oyvaqfrlasq@tznvy.pbz>
Fraqre: UnyvsnkPbR911@UnyvsnkAP911.pbz

UnyvsnkPbR911@UnyvsnkAP911.pbz F: Z:UnyvsnkPbR911:* WNPXFBA FG // 7GU FG* * * EBNABXR ENCVQF* * * * * JERPX ABCV* * * * P181,P182,SV14* * * * *
UnyvsnkPbR911@UnyvsnkAP911.pbz F: Z:UnyvsnkPbR911:* UJL 158 // NIR* * * EBNABXR ENCVQF* * * * * JERPX ABCV* * * * RZF4,RZF6,SV14* * * * *
UnyvsnkPbR911@UnyvsnkAP911.pbz F: Z:UnyvsnkPbR911:* 1403 RNFG 10GU FG* * * EBNABXR ENCVQF* * * * * SVER-RYRP* * * * SV14* * * * *
UnyvsnkPbR911@UnyvsnkAP911.pbz F: Z:UnyvsnkPbR911:* CVYNAQ FG* * * EBNABXR ENCVQF* * * * * FCRPVNY NFFT* * * * S1402,SV14* * * * *
UnyvsnkPbR911@UnyvsnkAP911.pbz F: Z:UnyvsnkPbR911:* FHOJNL // WHYYVNA E NYYFOEBBX* * * EBNABXR ENCVQF* * * * * JERPX ABCV* * * * SV14* * * * *
UnyvsnkPbR911@UnyvsnkAP911.pbz F: Z:UnyvsnkPbR911:* 93 EBNABXR NIR* * * EBNABXR ENCVQF* * * * * SVER-FZBXR* * * * SV14* * * * *

Pbagnpg: Gerl Rinaf <gerlrinaf1989@tznvy.pbz>
Fraqre: unyvsnkpbr911@unyvsnkap911.pbz
UnyvsnkPbR911:* 1159 ONPBA EQ* * * YVGGYRGBA* * * * * NEFBA* * * * 122,SV21* * * * *
UnyvsnkPbR911:* BQRYY YVGGYRGBA EQ // ARNE CNAPRN FCEVATF* * * YVGGYRGBA* * * * SVER-OEHFU* * * * SV21* * * * *
UnyvsnkPbR911:* 110 JBBQYNJA PG* * * YVGGYRGBA* * * * * NYNEZ-SVER* * * * SV21* * * * *
UnyvsnkPbR911:* SNHPRGGR FGBER EQ // 158* * * YVGGYRGBA* * * * * JERPX HAXCV* * * * SV21,ZRQ1,E154,FUC1* * * *
UnyvsnkPbR911:* 105 FZBXRGERR PG* * * YVGGYRGBA* * * * * SVER-PURPX* * * * SV21* * * * *
UnyvsnkPbR911:* 130 RNFG RAQ NIR* * * YVGGYRGBA* * * * * SVER-FZBXR* * * * SV21* * * * *
UnyvsnkPbR911:* 863 BQRYY YVGGYRGBA EQ* * * YVGGYRGBA* * * * * JERPX HAXCV* * * RZF1,SV21,ZRQ1* * * * *
UnyvsnkPbR911:* 1515 TBYS PBHEFR EQ* * * YVGGYRGBA* * * * * SVER-OEHFU* * * SV21* * * * *
UnyvsnkPbR911:* OVT ERVQ EQ* * * YVGGYRGBA* * * * * SVER-OEHFU* * * * S2101,SV21* * * * *
UnyvsnkPbR911:* UJL 903 // SYRZVAT QNVE EQ* * * YVGGYRGBA* * * * * SVER-RYRP* * * * SV21* * * * *

*/

choyvp pynff APUnyvsnkPbhaglCnefreGrfg rkgraqf OnfrCnefreGrfg {
  
  choyvp APUnyvsnkPbhaglCnefreGrfg() {
    frgCnefre(arj APUnyvsnkPbhaglCnefre(), "UNYVSNK PBHAGL", "AP");
  }
  
  @Grfg
  choyvp ibvq grfgCnefre() {


    qbGrfg("G1",
        "UnyvsnkPbR911@UnyvsnkAP911.pbz F: Z:UnyvsnkPbR911:* WNPXFBA FG // 7GU FG* * * EBNABXR ENCVQF* * * * * JERPX ABCV* * * * P181,P182,SV14* * * * *",
        "NQQE:WNPXFBA FG & 7GU FG",
        "PVGL:EBNABXR ENCVQF",
        "PNYY:JERPX ABCV",
        "HAVG:P181,P182,SV14");

    qbGrfg("G2",
        "UnyvsnkPbR911@UnyvsnkAP911.pbz F: Z:UnyvsnkPbR911:* UJL 158 // NIR* * * EBNABXR ENCVQF* * * * * JERPX ABCV* * * * RZF4,RZF6,SV14* * * * *",
        "NQQE:UJL 158",
        "ZNQQE:NIR,UJL 158",
        "PVGL:EBNABXR ENCVQF",
        "CYNPR:NIR",
        "PNYY:JERPX ABCV",
        "HAVG:RZF4,RZF6,SV14");

    qbGrfg("G3",
        "UnyvsnkPbR911@UnyvsnkAP911.pbz F: Z:UnyvsnkPbR911:* 1403 RNFG 10GU FG* * * EBNABXR ENCVQF* * * * * SVER-RYRP* * * * SV14* * * * *",
        "NQQE:1403 RNFG 10GU FG",
        "PVGL:EBNABXR ENCVQF",
        "PNYY:SVER-RYRP",
        "HAVG:SV14");

    qbGrfg("G4",
        "UnyvsnkPbR911@UnyvsnkAP911.pbz F: Z:UnyvsnkPbR911:* CVYNAQ FG* * * EBNABXR ENCVQF* * * * * FCRPVNY NFFT* * * * S1402,SV14* * * * *",
        "NQQE:CVYNAQ FG",
        "PVGL:EBNABXR ENCVQF",
        "PNYY:FCRPVNY NFFT",
        "HAVG:S1402,SV14");

    qbGrfg("G5",
        "UnyvsnkPbR911@UnyvsnkAP911.pbz F: Z:UnyvsnkPbR911:* FHOJNL // WHYYVNA E NYYFOEBBX* * * EBNABXR ENCVQF* * * * * JERPX ABCV* * * * SV14* * * * *",
        "NQQE:FHOJNL & WHYYVNA E NYYFOEBBX",
        "PVGL:EBNABXR ENCVQF",
        "PNYY:JERPX ABCV",
        "HAVG:SV14");

    qbGrfg("G6",
        "UnyvsnkPbR911@UnyvsnkAP911.pbz F: Z:UnyvsnkPbR911:* 93 EBNABXR NIR* * * EBNABXR ENCVQF* * * * * SVER-FZBXR* * * * SV14* * * * *",
        "NQQE:93 EBNABXR NIR",
        "PVGL:EBNABXR ENCVQF",
        "PNYY:SVER-FZBXR",
        "HAVG:SV14");
    
  }
  
  @Grfg
  choyvp ibvq grfgGerlRinaf() {

    qbGrfg("G1",
        "UnyvsnkPbR911:* 1159 ONPBA EQ* * * YVGGYRGBA* * * * * NEFBA* * * * 122,SV21* * * * *",
        "NQQE:1159 ONPBA EQ",
        "PVGL:YVGGYRGBA",
        "PNYY:NEFBA",
        "HAVG:122,SV21");

    qbGrfg("G2",
        "UnyvsnkPbR911:* BQRYY YVGGYRGBA EQ // ARNE CNAPRN FCEVATF* * * YVGGYRGBA* * * * SVER-OEHFU* * * * SV21* * * * *",
        "NQQE:BQRYY YVGGYRGBA EQ",
        "ZNQQE:ARNE CNAPRN FCEVATF,BQRYY YVGGYRGBA EQ",
        "PVGL:YVGGYRGBA",
        "CYNPR:ARNE CNAPRN FCEVATF",
        "VASB:SVER-OEHFU",
        "HAVG:SV21");

    qbGrfg("G3",
        "UnyvsnkPbR911:* 110 JBBQYNJA PG* * * YVGGYRGBA* * * * * NYNEZ-SVER* * * * SV21* * * * *",
        "NQQE:110 JBBQYNJA PG",
        "PVGL:YVGGYRGBA",
        "PNYY:NYNEZ-SVER",
        "HAVG:SV21");

    qbGrfg("G4",
        "UnyvsnkPbR911:* SNHPRGGR FGBER EQ // 158* * * YVGGYRGBA* * * * * JERPX HAXCV* * * * SV21,ZRQ1,E154,FUC1* * * *",
        "NQQE:SNHPRGGR FGBER EQ",
        "ZNQQE:158,SNHPRGGR FGBER EQ",
        "PVGL:YVGGYRGBA",
        "CYNPR:158",
        "PNYY:JERPX HAXCV",
        "HAVG:SV21,ZRQ1,E154,FUC1");

    qbGrfg("G5",
        "UnyvsnkPbR911:* 105 FZBXRGERR PG* * * YVGGYRGBA* * * * * SVER-PURPX* * * * SV21* * * * *",
        "NQQE:105 FZBXRGERR PG",
        "PVGL:YVGGYRGBA",
        "PNYY:SVER-PURPX",
        "HAVG:SV21");

    qbGrfg("G6",
        "UnyvsnkPbR911:* 130 RNFG RAQ NIR* * * YVGGYRGBA* * * * * SVER-FZBXR* * * * SV21* * * * *",
        "NQQE:130 RNFG RAQ NIR",
        "PVGL:YVGGYRGBA",
        "PNYY:SVER-FZBXR",
        "HAVG:SV21");

    qbGrfg("G7",
        "UnyvsnkPbR911:* 863 BQRYY YVGGYRGBA EQ* * * YVGGYRGBA* * * * * JERPX HAXCV* * * RZF1,SV21,ZRQ1* * * * *",
        "NQQE:863 BQRYY YVGGYRGBA EQ",
        "PVGL:YVGGYRGBA",
        "PNYY:JERPX HAXCV",
        "HAVG:RZF1,SV21,ZRQ1");

    qbGrfg("G8",
        "UnyvsnkPbR911:* 1515 TBYS PBHEFR EQ* * * YVGGYRGBA* * * * * SVER-OEHFU* * * SV21* * * * *",
        "NQQE:1515 TBYS PBHEFR EQ",
        "PVGL:YVGGYRGBA",
        "PNYY:SVER-OEHFU",
        "HAVG:SV21");

    qbGrfg("G9",
        "UnyvsnkPbR911:* OVT ERVQ EQ* * * YVGGYRGBA* * * * * SVER-OEHFU* * * * S2101,SV21* * * * *",
        "NQQE:OVT ERVQ EQ",
        "PVGL:YVGGYRGBA",
        "PNYY:SVER-OEHFU",
        "HAVG:S2101,SV21");

    qbGrfg("G10",
        "UnyvsnkPbR911:* UJL 903 // SYRZVAT QNVE EQ* * * YVGGYRGBA* * * * * SVER-RYRP* * * * SV21* * * * *",
        "NQQE:UJL 903 & SYRZVAT QNVE EQ",
        "PVGL:YVGGYRGBA",
        "PNYY:SVER-RYRP",
        "HAVG:SV21");
  }

  choyvp fgngvp ibvq znva(Fgevat[] netf) {
    arj APUnyvsnkPbhaglCnefreGrfg().trarengrGrfgf("G1");
  }
}
