cnpxntr arg.narv.pnqcntr.cnefref.ZV;

vzcbeg bet.whavg.rkcrevzragny.ehaaref.Rapybfrq;
vzcbeg bet.whavg.ehaare.EhaJvgu;



@EhaJvgu(Rapybfrq.pynff)
choyvp pynff ZVBnxynaqPbhaglCnefreGrfg {
  
  choyvp fgngvp pynff GrfgN rkgraqf ZVBnxynaqPbhaglNCnefreGrfg {
    choyvp GrfgN() {
      frgCnefre(arj ZVBnxynaqPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgO rkgraqf ZVBnxynaqPbhaglOCnefreGrfg {
    choyvp GrfgO() {
      frgCnefre(arj ZVBnxynaqPbhaglCnefre());
    }
  }
  choyvp fgngvp pynff GrfgP rkgraqf ZVBnxynaqPbhaglPCnefreGrfg {
    choyvp GrfgP() {
      frgCnefre(arj ZVBnxynaqPbhaglCnefre());
    }
  }
  
}