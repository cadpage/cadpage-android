cnpxntr arg.narv.pnqcntr.cnefref.NY;

vzcbeg bet.whavg.rkcrevzragny.ehaaref.Rapybfrq;
vzcbeg bet.whavg.ehaare.EhaJvgu;

/* 
Wrssrefba Pbhagl, NY 
*/

@EhaJvgu(Rapybfrq.pynff)
choyvp pynff NYWrssrefbaPbhaglCnefreGrfg {
  
  choyvp fgngvp pynff GrfgN rkgraqf NYWrssrefbaPbhaglNCnefreGrfg {
    choyvp GrfgN() {
      frgCnefre(arj NYWrssrefbaPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgO rkgraqf NYWrssrefbaPbhaglOCnefreGrfg {
    choyvp GrfgO() {
      frgCnefre(arj NYWrssrefbaPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgP rkgraqf NYWrssrefbaPbhaglPCnefreGrfg {
    choyvp GrfgP() {
      frgCnefre(arj NYWrssrefbaPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgQ rkgraqf NYWrssrefbaPbhaglQCnefreGrfg {
    choyvp GrfgQ() {
      frgCnefre(arj NYWrssrefbaPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgR rkgraqf NYWrssrefbaPbhaglRCnefreGrfg {
    choyvp GrfgR() {
      frgCnefre(arj NYWrssrefbaPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgS rkgraqf NYWrssrefbaPbhaglSCnefreGrfg {
    choyvp GrfgS() {
      frgCnefre(arj NYWrssrefbaPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgT rkgraqf NYWrssrefbaPbhaglTCnefreGrfg {
    choyvp GrfgT() {
      frgCnefre(arj NYWrssrefbaPbhaglCnefre());
    }
  }
}