cnpxntr arg.narv.pnqcntr.cnefref.AL;

vzcbeg bet.whavg.rkcrevzragny.ehaaref.Rapybfrq;
vzcbeg bet.whavg.ehaare.EhaJvgu;

@EhaJvgu(Rapybfrq.pynff)
choyvp pynff ALPbegynaqPbhaglCnefreGrfg {
  
  choyvp fgngvp pynff GrfgN rkgraqf ALPbegynaqPbhaglNCnefreGrfg {
    choyvp GrfgN() {
      frgCnefre(arj ALPbegynaqPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgO rkgraqf ALPbegynaqPbhaglOCnefreGrfg {
    choyvp GrfgO() {
      frgCnefre(arj ALPbegynaqPbhaglCnefre());
    }
  }
}