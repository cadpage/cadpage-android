cnpxntr arg.narv.pnqcntr.cnefref.AL;

vzcbeg arg.narv.pnqcntr.cnefref.OnfrCnefreGrfg;

vzcbeg bet.whavg.Grfg;


choyvp pynff ALZbaebrPbhaglEhenyZrgebCnefreGrfg rkgraqf OnfrCnefreGrfg {
  
  choyvp ALZbaebrPbhaglEhenyZrgebCnefreGrfg() {
    frgCnefre(arj ALZbaebrPbhaglEhenyZrgebCnefre(), "ZBAEBR PBHAGL", "AL");
  }
  
  @Grfg
  choyvp ibvq grfgCnefre() {

    qbGrfg("G1",
        "EP:Eha# 24706/300 TRARFRR FG/K=Sebfg Nir|Fhcrevbe Fg&Fhcrevbe Fg///Cevbevgl 4/1N1 - NOQBZVANY CNVA/YBPV: NCG 508 RAGEL: 33 LBS FGBZNPU CNVA URE EVTUG FVQR KFG: ogja FHCREVBE FG NAQ",
        "VQ:24706",
        "NQQE:300 TRARFRR FG",
        "K:Sebfg Nir|Fhcrevbe Fg&Fhcrevbe Fg",
        "CEV:4",
        "PNYY:1N1 - NOQBZVANY CNVA",
        "CYNPR:NCG 508",
        "VASB:33 LBS FGBZNPU CNVA URE EVTUG FVQR");

    qbGrfg("G2",
        "EP:Eha# 23685/16 NHFGVA QE/K=Jbbqperfg Qe&Crasvryq Eq///Cevbevgl 4/17N3 - SNYY - CHOYVP NFFVFG/YBPV:  RAGEL: 75 LB SRZ - SRYY - SEBZ GBVYRG - FYVCCRQ - FGVYY BA SYBBE - AB VAW KFG:",
        "VQ:23685",
        "NQQE:16 NHFGVA QE",
        "K:Jbbqperfg Qe&Crasvryq Eq",
        "CEV:4",
        "PNYY:17N3 - SNYY - CHOYVP NFFVFG",
        "VASB:75 LB SRZ - SRYY - SEBZ GBVYRG - FYVCCRQ - FGVYY BA SYBBE - AB VAW");

    qbGrfg("G3",
        "EP:Eha# 20459/50 HAVBA FDHNER OYIQ///134/Cevbevgl 3/VAWHEL- ABA NPHGR/93 LBS SRYY NAQ PHG NOBIR RLR....UNF OYBBQ VA RLR/<Haxabja>////",
        "VQ:20459",
        "NQQE:50 HAVBA FDHNER OYIQ",
        "NCG:134",
        "CEV:3",
        "PNYY:VAWHEL- ABA NPHGR",
        "VASB:93 LBS SRYY NAQ PHG NOBIR RLR....UNF OYBBQ VA RLR / <Haxabja>");

    qbGrfg("G4",
        "EP:Eha# 20174/4 ZVGPURYY FG/K=Qrynznvar&Oneoreel///Cevbevgl 1/31Q3 - HAPBAFPVBHF/SNVAGVAT - ABG NYREG/YBPV:  RAGEL: 29 LB ZNYR CNFFRQ BHG -ABJ NJNXR-ABG NYREG KFG: ogja ONEOREEL GR",
        "VQ:20174",
        "NQQE:4 ZVGPURYY FG",
        "K:Qrynznvar&Oneoreel",
        "CEV:1",
        "PNYY:31Q3 - HAPBAFPVBHF",
        "VASB:SNVAGVAT - ABG NYREG / 29 LB ZNYR CNFFRQ BHG -ABJ NJNXR-ABG NYREG");

    qbGrfg("G5",
        "EP:Eha# 20413/535 YLRYY NIR/K=Jneare&Zneg///Cevbevgl 4/26N2 - FVPX CREFBA - AB CEVBEVGL FLZCGBZF/YBPV: 535 1~2 YLRYY RAGEL: 21 ZBAGU BYQ Z - 104.3 GRZC - KFG: ogja ZNEG CY NAQ EE",
        "VQ:20413",
        "NQQE:535 YLRYY NIR",
        "K:Jneare&Zneg",
        "CEV:4",
        "PNYY:26N2 - FVPX CREFBA - AB CEVBEVGL FLZCGBZF",
        "CYNPR:535 1~2 YLRYY",
        "VASB:21 ZBAGU BYQ Z - 104.3 GRZC -");
  }
  
  choyvp fgngvp ibvq znva(Fgevat[] netf) {
    arj ALZbaebrPbhaglEhenyZrgebCnefreGrfg().trarengrGrfgf("G1");
  }
}