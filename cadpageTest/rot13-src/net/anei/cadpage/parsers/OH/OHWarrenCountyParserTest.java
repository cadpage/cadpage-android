cnpxntr arg.narv.pnqcntr.cnefref.BU;

vzcbeg bet.whavg.rkcrevzragny.ehaaref.Rapybfrq;
vzcbeg bet.whavg.ehaare.EhaJvgu;


@EhaJvgu(Rapybfrq.pynff)
choyvp pynff BUJneeraPbhaglCnefreGrfg {
  
  choyvp fgngvp pynff GrfgN rkgraqf BUJneeraPbhaglNCnefreGrfg {
    choyvp GrfgN() {
      frgCnefre(arj BUJneeraPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgO rkgraqf BUJneeraPbhaglOCnefreGrfg {
    choyvp GrfgO() {
      frgCnefre(arj BUJneeraPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgP rkgraqf BUJneeraPbhaglPCnefreGrfg {
    choyvp GrfgP() {
      frgCnefre(arj BUJneeraPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgQ rkgraqf BUJneeraPbhaglQCnefreGrfg {
    choyvp GrfgQ() {
      frgCnefre(arj BUJneeraPbhaglCnefre());
    }
  }
}