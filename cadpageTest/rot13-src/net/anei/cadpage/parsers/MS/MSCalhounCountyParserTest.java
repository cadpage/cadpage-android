cnpxntr arg.narv.pnqcntr.cnefref.ZF;

vzcbeg arg.narv.pnqcntr.cnefref.OnfrCnefreGrfg;

vzcbeg bet.whavg.Grfg;

/*
Pnyubha Pbhagl, ZF
Pbagnpg: Npgvir911
Fraaqre: pnqnyregf@rsbeprfbsgjner.pbz
Ntrapl anzr: Pnyubha Pbhagl 912 Ybpngvba: Cvggfobeb, ZF 

(Qvfcngpu Nyreg) Pnyy Glcr: ONAARE RZE\aPvgl: INEQNZNA\aNqqerff: 10 PE 117
(Qvfcngpu Nyreg) Pnyy Glcr: ONAARE RZE\aPvgl: INEQNZNA\aNqqerff: 123 A ZNVA FG
(Qvfcngpu Nyreg) Pnyy Glcr: ONAARE RZE\aPvgl: CVGGFOBEB\aNqqerff: 45 PE 104

*/

choyvp pynff ZFPnyubhaPbhaglCnefreGrfg rkgraqf OnfrCnefreGrfg {
  
  choyvp ZFPnyubhaPbhaglCnefreGrfg() {
    frgCnefre(arj ZFPnyubhaPbhaglCnefre(), "PNYUBHA PBHAGL", "ZF");
  }
  
  @Grfg
  choyvp ibvq grfgCnefre() {

    qbGrfg("G1",
        "(Qvfcngpu Nyreg) Pnyy Glcr: ONAARE RZE\aPvgl: INEQNZNA\aNqqerff: 10 PE 117",
        "PNYY:ONAARE RZE",
        "NQQE:10 PE 117",
        "ZNQQE:10 PBHAGL EBNQ 117",
        "PVGL:INEQNZNA");

    qbGrfg("G2",
        "(Qvfcngpu Nyreg) Pnyy Glcr: ONAARE RZE\aPvgl: INEQNZNA\aNqqerff: 123 A ZNVA FG",
        "PNYY:ONAARE RZE",
        "NQQE:123 A ZNVA FG",
        "PVGL:INEQNZNA");

    qbGrfg("G3",
        "(Qvfcngpu Nyreg) Pnyy Glcr: ONAARE RZE\aPvgl: CVGGFOBEB\aNqqerff: 45 PE 104",
        "PNYY:ONAARE RZE",
        "NQQE:45 PE 104",
        "ZNQQE:45 PBHAGL EBNQ 104",
        "PVGL:CVGGFOBEB");

  }
  
  choyvp fgngvp ibvq znva(Fgevat[] netf) {
    arj ZFPnyubhaPbhaglCnefreGrfg().trarengrGrfgf("G1", "PNYY NQQE K PVGL ANZR CUBAR ZNC VQ");
  }
}