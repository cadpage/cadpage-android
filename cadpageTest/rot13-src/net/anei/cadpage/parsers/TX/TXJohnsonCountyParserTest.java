cnpxntr arg.narv.pnqcntr.cnefref.GK;

vzcbeg arg.narv.pnqcntr.cnefref.OnfrCnefreGrfg;

vzcbeg bet.whavg.rkcrevzragny.ehaaref.Rapybfrq;
vzcbeg bet.whavg.ehaare.EhaJvgu;


@EhaJvgu(Rapybfrq.pynff)
choyvp pynff GKWbuafbaPbhaglCnefreGrfg rkgraqf OnfrCnefreGrfg {
  
  choyvp fgngvp pynff GrfgN rkgraqf GKWbuafbaPbhaglNCnefreGrfg {
    choyvp GrfgN() {
      frgCnefre(arj GKWbuafbaPbhaglNCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgO rkgraqf GKWbuafbaPbhaglOCnefreGrfg {
    choyvp GrfgO() {
      frgCnefre(arj GKWbuafbaPbhaglNCnefre());
    }
  }
  
}