cnpxntr arg.narv.pnqcntr.cnefref.GK;

vzcbeg arg.narv.pnqcntr.cnefref.OnfrCnefreGrfg;

vzcbeg bet.whavg.rkcrevzragny.ehaaref.Rapybfrq;
vzcbeg bet.whavg.ehaare.EhaJvgu;


@EhaJvgu(Rapybfrq.pynff)
choyvp pynff GKGneenagPbhaglCnefreGrfg rkgraqf OnfrCnefreGrfg {
  
  choyvp fgngvp pynff GrfgN rkgraqf GKGneenagPbhaglNCnefreGrfg {
    choyvp GrfgN() {
      frgCnefre(arj GKGneenagPbhaglCnefre());
    }
  }
  choyvp fgngvp pynff GrfgO rkgraqf GKGneenagPbhaglOCnefreGrfg {
    choyvp GrfgO() {
      frgCnefre(arj GKGneenagPbhaglCnefre());
    }
  }
}