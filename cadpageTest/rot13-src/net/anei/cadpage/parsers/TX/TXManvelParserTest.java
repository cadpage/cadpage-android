cnpxntr arg.narv.pnqcntr.cnefref.GK;

vzcbeg arg.narv.pnqcntr.cnefref.OnfrCnefreGrfg;

vzcbeg bet.whavg.Grfg;


choyvp pynff GKZnairyCnefreGrfg rkgraqf OnfrCnefreGrfg {
  
  choyvp GKZnairyCnefreGrfg() {
    frgCnefre(arj GKZnairyCnefre(), "ZNAIRY", "GK");
  }
  
  @Grfg
  choyvp ibvq grfgCnefre() {

    qbGrfg("G1",
        "[ ARJ PNYY ]  ZISQ\aNYSVER\a3403 FBHGUSBEX CNEXJNL\aZNAIRY\a1103200011 12:50 CZ",
        "FEP:ZISQ",
        "PNYY:NYSVER",
        "NQQE:3403 FBHGUSBEX CNEXJNL",
        "PVGL:ZNAIRY");

    qbGrfg("G2",
        "[ ARJ PNYY ]  ZISQ\aNPPHAX\aF. ZNFGRE / WHFG CNFG TNGRF YBBC\aZNAIRY\a1102210022 5:25 CZ",
        "FEP:ZISQ",
        "PNYY:NPPHAX",
        "NQQE:F ZNFGRE & WHFG CNFG TNGRF YBBC",
        "PVGL:ZNAIRY");

    qbGrfg("G3",
        "[ ARJ PNYY ]  ZISQ\aSVER\a13508 NHGHZA NFU PG\aZNAIRY\a1103030017 3:36 CZ",
        "FEP:ZISQ",
        "PNYY:SVER",
        "NQQE:13508 NHGHZA NFU PG",
        "PVGL:ZNAIRY");

    qbGrfg("G4",
        "[ ARJ PNYY ]  ZISQ\aSVER\a10516 YRRQL EQ\aZNAIRY\a1103010018 6:21 CZ",
        "FEP:ZISQ",
        "PNYY:SVER",
        "NQQE:10516 YRRQL EQ",
        "PVGL:ZNAIRY");

    qbGrfg("G5",
        "[ ARJ PNYY ]  ZISQ\aNYSVER\a3602 ORUYRE\aZNAIRY\a1102230010 3:30 CZ",
        "FEP:ZISQ",
        "PNYY:NYSVER",
        "NQQE:3602 ORUYRE",
        "PVGL:ZNAIRY");

    qbGrfg("G6",
        "[ ARJ PNYY ]  ZISQ\aSVER\a815 PE 393\aZNAIRY\a1102280014 2:47 CZ",
        "FEP:ZISQ",
        "PNYY:SVER",
        "NQQE:815 PE 393",
        "ZNQQE:815 PBHAGL EBNQ 393",
        "PVGL:ZNAIRY");

    qbGrfg("G7",
        "[ ARJ PNYY ]  ZISQ\aNPPHAX\a288/6\aZNAIRY\a1102220014 12:04 CZ",
        "FEP:ZISQ",
        "PNYY:NPPHAX",
        "NQQE:288 & 6",
        "PVGL:ZNAIRY");
    
    qbGrfg("G8",
        "ARJ PNYY / ZISQ\aSVER\a3110 RJVAT PG\aZNAIRY\a1103270030 3:37 CZ",
        "FEP:ZISQ",
        "PNYY:SVER",
        "NQQE:3110 RJVAT PG",
        "PVGL:ZNAIRY");

  }
  
  choyvp fgngvp ibvq znva(Fgevat[] netf) {
    arj GKZnairyCnefreGrfg().trarengrGrfgf("G1");
  }
}
