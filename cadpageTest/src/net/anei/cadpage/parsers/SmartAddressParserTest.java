...
public class SmartAddressParserTest extends BaseParserTest {
  ...
  private static final int FLAG_NEAR_TO_END = SmartAddressParser.FLAG_NEAR_TO_END;
  private static final int FLAG_PAD_FIELD_EXCL_CITY = SmartAddressParser.FLAG_PAD_FIELD_EXCL_CITY;
  
  private static final String[] CITY_LIST = new String[]{"KENSBURG", "KEN TOWN", "9999", "N BROOKFIELD"};
  ...
  public SmartAddressParserTest() {
    ...
    parser.setupMultiWordStreets("JEAN GOUL", "PAUL G GETTY", "CAPE ST CLAIRE");
  ...
  public void testProblems() {
    ...
    doTest(PLACE, FLAG_START_FLD_NO_DELIM | FLAG_ANCHOR_END, "12 BOWDEN LA",
        "ADDR:12 BOWDEN LA");
    
    doTest(CALL, FLAG_START_FLD_REQ | FLAG_ANCHOR_END, "JCSO MVC PROPERTY DAMAGE NE FF HWY & NE 50 HWY KEN TOWN",
    ...
    doTest(ADDR, FLAG_PAD_FIELD_EXCL_CITY, "316 HUFF ST 2B KENSBURG C9 VIC DWN TRACY KEN TOWN",
        "ADDR:316 HUFF ST",
        "APT:2B",
        "CITY:KENSBURG");
    
    doTest(PLACE , FLAG_NO_IMPLIED_APT, "HIGHWAY W & RIVERS END DR 2417 2434",
        "ADDR:HIGHWAY W & RIVERS END DR");
    
    doTest(CALL, FLAG_START_FLD_REQ, "1 ET TO COVER STA 71 Misc 6 WESTFORD RD",
        "CALL:1 ET TO COVER STA 71 Misc", 
        "ADDR:6 WESTFORD RD");
        
    doTest(CALL, "STRUCT FIRE RESID ST CATHERINE ST  KEN TOWN",
        "CALL:STRUCT FIRE RESID",
        "ADDR:ST CATHERINE ST",
        "CITY:KEN TOWN");
    
    doTest(ADDR, FLAG_PAD_FIELD, "10 RESERVOIR MNR Apt R5  KENSBURG BACK DOOR",
        "ADDR:10 RESERVOIR MNR",
        "APT:R5",
        "CITY:KENSBURG");
    
    doTest(ADDR, "0 OLD HWY 68 A THE OLD ROADSIDE PARK",
        "ADDR:OLD HWY 68");
    
    doTest(ADDR, "XX HWY 9 / GERTON HWY KENSBURG C21R BLKD-RD SCOTT MOORE",
        "ADDR:XX HWY 9 & GERTON HWY",
        "CITY:KENSBURG");
    
    doTest(ADDR, FLAG_CROSS_FOLLOWS, "HIGHWAY 43 & I 44 EB",
        "ADDR:HIGHWAY 43 & I 44 EB");
    
    doTest(CALL, "10-50 Rollover 4062 N CR 225 W",
        "CALL:10-50 Rollover", 
        "ADDR:4062 N CR 225 W");
    
    doTest(ADDR, FLAG_ONLY_CROSS | FLAG_CROSS_FOLLOWS, "KUYKENDAHL RD S ROLLING O",
        "X:KUYKENDAHL RD");
    
    doTest(ADDR, FLAG_CHECK_STATUS | FLAG_ANCHOR_END, "400 VALLEY MEADOW CI APT B1",
           "ADDR:400 VALLEY MEADOW CI",
           "APT:B1");
    
    doTest(ADDR, "RT50 W/CAPE ST CLAIRE RD QPM1,E191,QE1,QAFD MVC",
           "ADDR:RT50 W & CAPE ST CLAIRE RD");
    
    doTest(CALL, "CARB AUTO / KARIKAS RESD / 816532620 382 ANDREWS ROAD",
           "CALL:CARB AUTO / KARIKAS RESD / 816532620",
           "ADDR:382 ANDREWS ROAD");
    
    doTest(CALL, FLAG_START_FLD_REQ | FLAG_ANCHOR_END |FLAG_PAD_FIELD, 
           "Diabetic Problems 206 EARL DR DEAD END / VARNELL RD, LINGERFELT RD KEN TOWN",
           "CALL:Diabetic Problems",
           "ADDR:206 EARL DR",
           "SRC:DEAD END / VARNELL RD, LINGERFELT RD",
           "CITY:KEN TOWN");
    
    doTest(ADDR, FLAG_IMPLIED_INTERSECT | FLAG_ANCHOR_END, "HACKETT RD KANSAS RD S",
           "ADDR:HACKETT RD & KANSAS RD S");
    
    doTest(PLACE, FLAG_ANCHOR_END, "KENSBURG",
           "CITY:KENSBURG");
    
    doTest(PLACE, FLAG_ANCHOR_END, "KENSBURG AIRPORT KENSBURG",
           "PLACE:KENSBURG AIRPORT",
           "CITY:KENSBURG");
...
//                 "ADDR:20 HWY SERGEANT RD");
  ...
  public void testCrossStreets() {
    doTest(ADDR, "1000 SUNBURY ONCE XS: IN TOWN KENSBURG JUNK",
  ...
  public void testIntersections() {
    ...
    doTest(SKIP, "SOMEWHERE CLOSE TO US50 N & BLACK ST DOWN",
  ...
  public void testParseFallback() {
    ...
    doTest(CALL, FLAG_AT_BOTH, "SOMETIMES THINGS JUST DONT WORK",
        "CALL:SOMETIMES THINGS JUST DONT WORK");
  ...
  public void testFlexAtAddress() {
    ...
    doTest(CALL, FLAG_AT_BOTH, "YOUR PROBLEM N GREEN HILL RD @ CLEMINTINE JOHNSON",
        ...
        "ADDR:N GREEN HILL RD",
        "PLACE:CLEMINTINE");
    doTest(CALL, FLAG_AT_BOTH | FLAG_ANCHOR_END, "TRANSFER STATION 46 @1902 CHURCH CREEK RD",
  ...
  public void testGpsCoords() {
    ...
    doTest(CALL, "VEHICLE ACCIDENT LL(-77 59.6013,39 53.1520) DOWN TOWN",
        "CALL:VEHICLE ACCIDENT",
        "ADDR:LL(-77 59.6013,39 53.1520)");
  }
  ...
  public void testImpliedIntersection() {
    ...
    doTest(CALL, FLAG_IMPLIED_INTERSECT, "Vehicle Fire 41 ST SE 148 AVE SE KENSBURG",
        "CALL:Vehicle Fire",
        "ADDR:41 ST SE & 148 AVE SE",
        "CITY:KENSBURG");
  }
  ...
  public void testCrossOnly() {
    ...
    doTest(CALL, FLAG_ONLY_CROSS, "Sick person Dead End leftover",
        "CALL:Sick person",
        "X:Dead End");
    
    doTest(CALL, FLAG_ONLY_CROSS, "Sick person Dead End/Swan Dr leftover",
        "CALL:Sick person",
        "X:Dead End / Swan Dr");
    
    doTest(CALL, FLAG_ONLY_CROSS, "Sick person Swan Dr/Dead End leftover",
        "CALL:Sick person",
        "X:Swan Dr / Dead End");
    
    doTest(CALL, FLAG_ONLY_CROSS | FLAG_ANCHOR_END, "Sick person Dead End/Swan Dr",
        "CALL:Sick person",
        "X:Dead End / Swan Dr");
    
    doTest(CALL, FLAG_ONLY_CROSS | FLAG_ANCHOR_END, "Sick person Simpson Rd/E I-485 INNER HY",
        "CALL:Sick person",
  ...
  public void testOnlyCity() {
    ...
    doTest(ADDR, FLAG_ONLY_CITY, "KEN TOWN OVER",
        "CITY:KEN TOWN");
    
    doTest(CALL, FLAG_ONLY_CITY, "BAD WOLF KEN TOWN OVER",
        "CALL:BAD WOLF",
        "CITY:KEN TOWN");
    
    doTest(CALL, FLAG_ONLY_CITY, "TEEPEE MHP",
        "CALL:TEEPEE MHP");
    
    doTest(ADDR, FLAG_ONLY_CITY, "TEEPEE MHP");
    
    doTest(ADDR, FLAG_ONLY_CITY | FLAG_CROSS_FOLLOWS, "KENSBURG LANE APTS",
        "CITY:KENSBURG");
    
    doTest(ADDR, FLAG_ONLY_CITY, "KEN TOWN IS OVER THE HILL",
        "CITY:KEN TOWN");
    
    doTest(ADDR, FLAG_ONLY_CITY, "IS KEN TOWN IS OVER THE HILL");
  ...
  @Test
  public void testNearFields() {
    doTest(ADDR,"CO HWY 48 NEAR HWY 231 KENSBURG",
        "ADDR:CO HWY 48", 
        "X:HWY 231",
        "CITY:KENSBURG");
    doTest(ADDR,"CO HWY 48 NEAR BRIGHT HILL KENSBURG",
        "ADDR:CO HWY 48", 
        "PLACE:NEAR BRIGHT HILL",
        "CITY:KENSBURG");
    doTest(ADDR, "CO HWY 48 NEAR SUNSHINE CITY",
        "ADDR:CO HWY 48");
    doTest(ADDR, FLAG_NEAR_TO_END, "CO HWY 48 NEAR SUNSHINE CITY",
        "ADDR:CO HWY 48",
        "PLACE:NEAR SUNSHINE CITY");
    doTest(ADDR, FLAG_NEAR_TO_END, "CO HWY 48 NEAR BLACK ST VERIZON",
        "ADDR:CO HWY 48",
        "X:BLACK ST");
    doTest(ADDR, FLAG_NEAR_TO_END, "HWY 75 NEAR SOUTHEASTERN XS: REMLAP DR KENSBURG VERIZON WIRELESS",
        "ADDR:HWY 75",
  ...
  public void testRelation() {
    doTest(CALL, "ABDOM PAIN FAINT > 50 9535 BLILEY RD",
        "CALL:ABDOM PAIN FAINT > 50",
        "ADDR:9535 BLILEY RD");
    doTest(CALL, "BP < 40 125 KINGS CENTER",
        "CALL:BP < 40",
        "ADDR:125 KINGS CENTER");
  }
  
  @Test
  public void testCommaApt() {
    
    doTest(ADDR, "611 PINE ST, APT B KEN TOWN",
        "ADDR:611 PINE ST",
        "APT:B",
        "CITY:KEN TOWN");
    
    doTest(ADDR, "611 PINE ST, #B KEN TOWN",
        "ADDR:611 PINE ST",
        "APT:B",
        "CITY:KEN TOWN");
    
    doTest(ADDR, "611 PINE ST XS:BLACK AVE, APT B KEN TOWN",
        "ADDR:611 PINE ST",
        "X:BLACK AVE",
        "APT:B",
        "CITY:KEN TOWN");
    
    doTest(ADDR, "611 PINE ST XS:BLACK AVE, #B KEN TOWN",
        "ADDR:611 PINE ST",
        "X:BLACK AVE",
        "APT:B",
        "CITY:KEN TOWN");
    
    doTest(ADDR, FLAG_PAD_FIELD, "611 PINE ST APT A LOST WORLD KEN TOWN",
        "ADDR:611 PINE ST",
        "APT:A",
        "SRC:LOST WORLD",
        "CITY:KEN TOWN");
  ...
  @Test
  public void testPreservePrefix() {
    
    doTest(PLACE, "BLACK&DECKER 501 GREEN ST",
        "PLACE:BLACK&DECKER",
        "ADDR:501 GREEN ST");
    
    doTest(PLACE, "DEWEY,CHEATUM, & HOWE 501 GREEN ST",
        "PLACE:DEWEY,CHEATUM, & HOWE",
        "ADDR:501 GREEN ST");
  }
  ...
  public void testBypass() {
    ...
    doTest(CALL, "HELLO 1030 US 20 BYPASS AND OTHER STUFF",
        "CALL:HELLO",
        "ADDR:1030 US 20 BYPASS");

    doTest(CALL, "HELLO US 20 BYPASS & BLACK ST BEYOND THE HORIZON",
        "CALL:HELLO",
        "ADDR:US 20 BYPASS & BLACK ST");
    
    doTest(CALL, "HELLO EASTON BYP N / ST MICHAELS RD BEYOND",
        "CALL:HELLO",
