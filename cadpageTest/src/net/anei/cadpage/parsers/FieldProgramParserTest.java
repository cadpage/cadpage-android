...
public class FieldProgramParserTest extends BaseParserTest {
  ...
  public FieldProgramParserTest() {
    parser = new TestParser(CITY_LIST, DEF_CITY, DEF_STATE, null, 0);
  ...
  public void testconditionalBranch() {
    ...
    doFieldTest("untag-tag succ",
        "( INFO ID | NAME ) Unit:UNIT",
        "INFO;666;Unit:AAA",
        "INFO:INFO",
        "ID:666",
        "UNIT:AAA");
    
    doFieldTest("untag-tag fail",
        "( INFO ID | NAME ) Unit:UNIT",
        "KEN;Unit:AAA",
        "NAME:KEN",
        "UNIT:AAA");
    ...
    doFieldTest("untag-tag succ",
        "( INFO APT | NAME ) Unit:UNIT",
        "INFO;Apt: 20;Unit:AAA",
        "INFO:INFO",
        "APT:20",
        "UNIT:AAA");
    
    doFieldTest("branch-cond-repeat",
        "( ID PLACE | ) INFO+? SRC CITY!",
        "999;JOES BAR;XXX;KEN TOWN",
        "ID:999",
        "PLACE:JOES BAR",
        "SRC:XXX",
        "CITY:KEN TOWN");
  ...
  public void testOptionalTagFields() {
    ...
    doFieldTest("DoubleTag-1", "CALL Apt:APT? CITY Map:MAP",
        "BIG FIRE;KENTOWN;Map:X21",
        "CALL:BIG FIRE",
        "CITY:KENTOWN",
        "MAP:X21");
    
    doFieldTest("DoubleTag-2", "CALL Apt:APT? CITY Map:MAP",
        "BIG FIRE;Apt:B;KENTOWN;Map:X21",
        "CALL:BIG FIRE",
        "APT:B",
        "CITY:KENTOWN",
        "MAP:X21");
  ...
  public void testAnyOrder() {
    doFieldTest("AnyOrder fields", "CALL:CALL ADDRESS:ADDR CROSS:X UNIT:UNIT", 
        FieldProgramParser.FLDPROG_ANY_ORDER | FieldProgramParser.FLDPROG_IGNORE_CASE,
        "Cross:CROSS ST;Address:5 BLACK ST;Call:BAD NEWS;Unit:X15",
        "X:CROSS ST",
        "ADDR:5 BLACK ST",
        "CALL:BAD NEWS",
        "UNIT:X15");
    
    doFieldFail("AnyOrder fields", "CALL:CALL ADDRESS:ADDR CROSS:X UNIT:UNIT", 
        FieldProgramParser.FLDPROG_ANY_ORDER | FieldProgramParser.FLDPROG_IGNORE_CASE,
        "Cross:CROSS ST;Address:5 BLACK ST;Call:BAD NEWS;Unit:X15;BAD FIELD");
    
    doFieldFail("AnyOrder fields", "CALL:CALL ADDRESS:ADDR CROSS:X UNIT:UNIT", 
        FieldProgramParser.FLDPROG_ANY_ORDER | FieldProgramParser.FLDPROG_IGNORE_CASE,
        "Cross:CROSS ST;Address:5 BLACK ST;Call:BAD NEWS;Unit:X15;BAD:BAD FIELD");
  }
 
  @Override
  public void testBadMsg() {
    parser.setProgram("SKIP ADDR!", 0);
    super.testBadMsg();
  }

  private void doFieldTest(String title, String program, String body, String ... result) {
    doFieldTest(title, program, 0, body, result);
  }

  private void doFieldTest(String title, String program, int flags, String body, String ... result) {
    parser.setProgram(program, flags);
    parser.checkForSkips();
    doTest(title, body, result);
  }
  
  private void doFieldFail(String title, String program, String body) {
    doFieldFail(title, program, 0, body);
  }
  
  private void doFieldFail(String title, String program, int flags, String body) {
    parser.setProgram(program, flags);
    parser.checkForSkips();
  ...
  private static class TestParser extends FieldProgramParser {
    ...
    public TestParser(String[] cities, String defCity, String defState, String programStr, int flags) {
      super(cities, defCity, defState, programStr, flags);
    }
