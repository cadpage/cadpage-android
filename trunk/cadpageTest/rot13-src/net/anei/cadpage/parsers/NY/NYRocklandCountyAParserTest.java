cnpxntr arg.narv.pnqcntr.cnefref.AL;

vzcbeg arg.narv.pnqcntr.cnefref.OnfrCnefreGrfg;

vzcbeg bet.whavg.Grfg;

/*
Ebpxynaq Pbhagl, AL
Pbagnpg: "Gvzbgul N Jbbyyrl" <GJbbyrl@arjpvglsver.bet>
Fraqre: cntvat@nycvarfbsgjner.pbz
Flfgrz: ErqNyreg

NHGBZNGVP NYNEZ ng 254 F ZNVA FG, Arj Pvgl p/f:RYVABE CY FPUEVRIRE YA . . 18:59:20
ANGHENY TNF - BHGFVQR ng 108 OVEPU YA, Arj Pvgl p/f: BYQ OEVPX EQ / ERQ UVYYEQ . . 22:15:43
NHGBZNGVP NYNEZ ng 129 FGENJGBJA EQ, Arj Pvgl p/f: IVYYNTR JNL / EBAR PG . . 00:07:21
IRUVPYR SVER ng 48 TREZBAQF EQ, Arj Pvgl p/f: EBHGR 304 / YLAPERFG NI . . 15:14:38
PB J/ FLZCGBZF ng 85 BYQ CUVYYVCF UVYY EQ, Arj Pvgl p/f: ZNUBARL PG / OHRAN IVFGN EQ . . 18:18:51
OEHFU/ZHYPU/EHOOVFU BHGFVQR ng 14 BYQ EBHGR 304, Arj Pvgl p/f: SNVE UNIRA QE, JRFGTNGR OYIQ / A ZNVA FG, MHXBE EQ 16:16:06
UNNMZNG ng PBATREF EQ/ZNVA FG, Arj Pvgl . . 10:40:52

Pbagnpg: puevf byvin <puevfbyvin1@tznvy.pbz>
Fraqre: arjpvglpnq@erqazkpnq.pbz
  / NYNEZ FBHAQVAT ng 254 F ZNVA FG, Arj Pvgl  p/f: RYVABE CY / FPUEVRIRE YA . . 14:01:52\a

Pbagnpg: Revp <rfxnyn@zbafrlsq.bet>
Fraqre: zbafrlsq@erqazkpnq.pbz
BIRA/NCCYVNAPR/PBBXVAT SVER ng 23 EVGN NI, Xnfre  p/f: WVYY YA / NFURY YA . . 16:58:34

*/

choyvp pynff ALEbpxynaqPbhaglNCnefreGrfg rkgraqf OnfrCnefreGrfg {
  
  choyvp ALEbpxynaqPbhaglNCnefreGrfg() {
    frgCnefre(arj ALEbpxynaqPbhaglNCnefre(), "EBPXYNAQ PBHAGL", "AL");
  }
  
  @Grfg
  choyvp ibvq grfgCnefre() {

    qbGrfg("G1",
        "NHGBZNGVP NYNEZ ng 254 F ZNVA FG, Arj Pvgl p/f:RYVABE CY FPUEVRIRE YA . . 18:59:20",
        "PNYY:NHGBZNGVP NYNEZ",
        "NQQE:254 F ZNVA FG",
        "PVGL:Arj Pvgl",
        "K:RYVABE CY FPUEVRIRE YA",
        "GVZR:18:59:20");

    qbGrfg("G2",
        "ANGHENY TNF - BHGFVQR ng 108 OVEPU YA, Arj Pvgl p/f: BYQ OEVPX EQ / ERQ UVYYEQ . . 22:15:43",
        "PNYY:ANGHENY TNF - BHGFVQR",
        "NQQE:108 OVEPU YA",
        "PVGL:Arj Pvgl",
        "K:BYQ OEVPX EQ / ERQ UVYYEQ",
        "GVZR:22:15:43");

    qbGrfg("G3",
        "NHGBZNGVP NYNEZ ng 129 FGENJGBJA EQ, Arj Pvgl p/f: IVYYNTR JNL / EBAR PG . . 00:07:21",
        "PNYY:NHGBZNGVP NYNEZ",
        "NQQE:129 FGENJGBJA EQ",
        "PVGL:Arj Pvgl",
        "K:IVYYNTR JNL / EBAR PG",
        "GVZR:00:07:21");

    qbGrfg("G4",
        "IRUVPYR SVER ng 48 TREZBAQF EQ, Arj Pvgl p/f: EBHGR 304 / YLAPERFG NI . . 15:14:38",
        "PNYY:IRUVPYR SVER",
        "NQQE:48 TREZBAQF EQ",
        "PVGL:Arj Pvgl",
        "K:EBHGR 304 / YLAPERFG NI",
        "GVZR:15:14:38");

    qbGrfg("G5",
        "PB J/ FLZCGBZF ng 85 BYQ CUVYYVCF UVYY EQ, Arj Pvgl p/f: ZNUBARL PG / OHRAN IVFGN EQ . . 18:18:51",
        "PNYY:PB J/ FLZCGBZF",
        "NQQE:85 BYQ CUVYYVCF UVYY EQ",
        "PVGL:Arj Pvgl",
        "K:ZNUBARL PG / OHRAN IVFGN EQ",
        "GVZR:18:18:51");

    qbGrfg("G6",
        "OEHFU/ZHYPU/EHOOVFU BHGFVQR ng 14 BYQ EBHGR 304, Arj Pvgl p/f: SNVE UNIRA QE, JRFGTNGR OYIQ / A ZNVA FG, MHXBE EQ 16:16:06",
        "PNYY:OEHFU/ZHYPU/EHOOVFU BHGFVQR",
        "NQQE:14 BYQ EBHGR 304",
        "PVGL:Arj Pvgl",
        "K:SNVE UNIRA QE",
        "VASB:JRFGTNGR OYIQ / A ZNVA FG, MHXBE EQ 16:16:06");

    qbGrfg("G7",
        "UNNMZNG ng PBATREF EQ/ZNVA FG, Arj Pvgl . . 10:40:52",
        "PNYY:UNNMZNG",
        "NQQE:PBATREF EQ & ZNVA FG",
        "PVGL:Arj Pvgl",
        "GVZR:10:40:52");

    qbGrfg("G8",
        "  / NYNEZ FBHAQVAT ng 254 F ZNVA FG, Arj Pvgl  p/f: RYVABE CY / FPUEVRIRE YA . . 14:01:52\a",
        "PNYY:NYNEZ FBHAQVAT",
        "NQQE:254 F ZNVA FG",
        "PVGL:Arj Pvgl",
        "K:RYVABE CY / FPUEVRIRE YA",
        "GVZR:14:01:52");

    qbGrfg("G9",
        "BIRA/NCCYVNAPR/PBBXVAT SVER ng 23 EVGN NI, Xnfre  p/f: WVYY YA / NFURY YA . . 16:58:34",
        "PNYY:BIRA/NCCYVNAPR/PBBXVAT SVER",
        "NQQE:23 EVGN NI",
        "ZNQQE:23 EVGN NIR",
        "PVGL:Xnfre",
        "K:WVYY YA / NFURY YA",
        "GVZR:16:58:34");

  }
  
  choyvp fgngvp ibvq znva(Fgevat[] netf) {
    arj ALEbpxynaqPbhaglNCnefreGrfg().trarengrGrfgf("G1", "PNYY NQQE PVGL K GVZR");
  }
}