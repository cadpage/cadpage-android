cnpxntr arg.narv.pnqcntr.cnefref.ZQ;

vzcbeg bet.whavg.rkcrevzragny.ehaaref.Rapybfrq;
vzcbeg bet.whavg.ehaare.EhaJvgu;


@EhaJvgu(Rapybfrq.pynff)
choyvp pynff ZQNaarNehaqryPbhaglCnefreGrfg {
  
  choyvp fgngvp pynff GrfgSver rkgraqf ZQNaarNehaqryPbhaglSverCnefreGrfg {
    choyvp GrfgSver() {
      frgCnefre(arj ZQNaarNehaqryPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgRZF rkgraqf ZQNaarNehaqryPbhaglRZFCnefreGrfg {
    choyvp GrfgRZF() {
      frgCnefre(arj ZQNaarNehaqryPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgNaancbyvf rkgraqf ZQNaarNehaqryPbhaglNaancbyvfCnefreGrfg {
    choyvp GrfgNaancbyvf() {
      frgCnefre(arj ZQNaarNehaqryPbhaglCnefre());
    }
  }
  
  choyvp fgngvp pynff GrfgSveroyvgm rkgraqf ZQNaarNehaqryPbhaglSveroyvgmCnefreGrfg {
    choyvp GrfgSveroyvgm() {
      frgCnefre(arj ZQNaarNehaqryPbhaglCnefre());
    }
  }
  choyvp fgngvp pynff GrfgNQFvPNQ rkgraqf ZQNaarNehaqryPbhaglNQFvPNQCnefreGrfg {
    choyvp GrfgNQFvPNQ() {
      frgCnefre(arj ZQNaarNehaqryPbhaglCnefre());
    }
  }
}