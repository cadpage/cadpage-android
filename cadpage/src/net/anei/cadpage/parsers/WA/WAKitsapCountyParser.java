...
public class WAKitsapCountyParser extends SmartAddressParser {
  ...
  public static Pattern MID = Pattern.compile("(?:(.*?)  +(.*?)(?: +- +[NSEW]{1,2} .*?| +CALLBK=.*?)?  +([A-Z]?\\(?\\d{3}\\)?[ \\-]?\\d{3}[\\- ]\\d{4})|(.*?)) *(?:T:(\\d{2}:\\d{2}:\\d{2})(?: C:(?:([+\\-]\\d+(?:\\.\\d+)?) ?([+\\-]\\d+(?:\\.\\d+)?))? *(.*))?)?");
  ...
  public boolean parseMsg(String subject, String body, Data data) {
    ...
    if (loc != null) parseAddress(StartType.START_ADDR, FLAG_ANCHOR_END, loc.trim(), data);
    ...
    data.strGPSLoc = append(getOptGroup(mat.group(7)), " ", getOptGroup(mat.group(6))); //flip them
    body = mat.group(8);
