...
/**
 * Code table containing the predefined standard medical and fire codes
 */
public class StandardCodeTable extends CodeTable {
  ...
  public String getCodeDescription(String code) {
    return super.getCodeDescription(expand(code));
  }
  ...
  private String expand(String code) {
    ...
    if (! Character.isDigit(code.charAt(1))) code = "0" + code;
    else if (code.length() < 4) return code;
    if (Character.isDigit(code.charAt(3))) {
      if (code.length() == 4 || !Character.isDigit(code.charAt(4))) {
