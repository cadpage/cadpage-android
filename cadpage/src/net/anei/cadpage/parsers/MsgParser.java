...
public abstract class MsgParser {
  ...
  public static final Pattern GPS_PATTERN = 
      Pattern.compile("(?:\\b|^|[ ,;\\.]+)(X: *|LAT: *|LL\\( *)?[NS]?(" + GPS_COORD_PTN_STR2 + ")[NnSs]?[,\\W] ?\\W*?(Y: *|LON: *|x )?[EW]?(" + GPS_COORD_PTN_STR2 + ")[EW]?(?:\\)|\\b)", Pattern.CASE_INSENSITIVE);
...
public static void addCodeTable(Properties props, String[] table) {
 ...
 private static final Pattern WIRELESS_CARRIER_PTN = Pattern.compile("\\b(?:WIRELESS-)?(?:VERIZON(?: WIRELESS)?(?: INTRADO \\(PSAP\\))?(?:\\(XYP\\))?|VZW(?:M?OBILE)?(?: USA)?|(?:MOBILITY,? )?(?:ATT? ?& ?T|ATT)(?: MOBILITY)?(?: \\(TCS\\))?|ATTMO|CONNEXON|T-MOBILE|SPRINT(?:PCS)?(?: NEXTEL- CDMA)?|US CELLULAR|METRO ?PCS|CORR WIRELESS|BLUEGRASS CELLULAR|APPALACHIAN WIRELESS|STARR WIRELESS)\\b.*", Pattern.CASE_INSENSITIVE);
