...
public class DispatchA42Parser extends SmartAddressParser {
  ...
  private static Pattern TAGGED_MASTER = Pattern.compile("Location: *(.*?) *(?:(-?\\d{1,3}\\.\\d+ -?\\d{1,3}\\.\\d+)|-361 -361) *(.*?) *Common Name: *(.*?) *Call Type: *(.*?) *Call Time: *(\\d{2}/\\d{2}/\\d{2}) *(\\d{2}:\\d{2}) *Call Number: *-?(\\d+) *Narrative: *(.*?)");
