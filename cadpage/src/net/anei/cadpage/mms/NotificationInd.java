...
public class NotificationInd extends GenericPdu {
    ...
    public SmsMmsMessage getMessage() {
      MessageClass msgCls = MessageClass.UNKNOWN;
      byte[] baMsgCls = getMessageClass();
      if (baMsgCls != null) {
        try {
          msgCls = MessageClass.valueOf(new String(baMsgCls));
        } catch (Exception ex) {}
      }
      String from = getStringValue(getFrom());
      String subject = getStringValue(getSubject());
      String content = getStringValue(getContentLocation());
      String msgId = getStringValue(getTransactionId());
